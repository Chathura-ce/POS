...
class SyncPushRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'suppliers.*.id' => 'required|uuid',
            'suppliers.*.name' => 'required|string|max:255',
            'suppliers.*.phone' => 'nullable|string|max:50',
            'suppliers.*.email' => 'nullable|email|max:255',
            'suppliers.*.balance' => 'required|numeric|min:0',
            'suppliers.*.updated_at' => 'required|date',
            'purchase_orders' => 'sometimes|array',
            'purchase_orders.*.id' => 'required|uuid',
            'purchase_orders.*.supplier_id' => 'required|uuid',
            'purchase_orders.*.total_amount' => 'required|numeric|min:0',
            'purchase_orders.*.status' => 'required|in:pending,received',
            'purchase_orders.*.created_at' => 'required|date',
            'purchase_orders.*.items' => 'required|array|min:1',
            'purchase_orders.*.items.*.id' => 'required|uuid',
            'purchase_orders.*.items.*.product_id' => 'required|uuid',
            'purchase_orders.*.items.*.quantity' => 'required|integer|min:1',
            'purchase_orders.*.items.*.unit_cost' => 'required|numeric|min:0',
            'purchase_orders.*.items.*.line_total' => 'required|numeric|min:0',
            'supplier_payments' => 'sometimes|array',
            'supplier_payments.*.id' => 'required|uuid',
            'supplier_payments.*.supplier_id' => 'required|uuid',
            'supplier_payments.*.amount' => 'required|numeric|min:0',
        ];
    }
}
